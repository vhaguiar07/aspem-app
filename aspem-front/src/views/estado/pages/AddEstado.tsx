...
import { Estado, AdventicioEstado, DentCrossEstado, OdMedEstado, RioPaxEstado, DependenteEstado, CooperadorEstado } from '../types';
import { getEstadoById, createEstado } from '../estadoApi';
import { Button, FormGroup, Intent, InputGroup } from '@blueprintjs/core';
import { ptBR } from 'date-fns/locale';
import { parse } from 'date-fns';
import './addEstadoStyles.css';
import './nice-form.css'
import { addEstadoSuccess, addEstadoFailure } from '../reducer';

const AddEstado: React.FC = () => {
  ...
  const [adventiciosEstado] = useState<AdventicioEstado[]>([]);
  const [cooperadoresEstado] = useState<CooperadorEstado[]>([]);
  const [dentCrossesEstado] = useState<DentCrossEstado[]>([]);
  const [odMedsEstado] = useState<OdMedEstado[]>([]);
  const [rioPaxesEstado] = useState<RioPaxEstado[]>([]);
  const [dependentesEstado] = useState<DependenteEstado[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    estadoToSend.quantidadeAdventicios = estado.quantidadeAdventicios ?? 0;

    if (adventiciosEstado.length === 0) {
      delete estadoToSend.adventiciosEstado;
    } else {
      estadoToSend.adventiciosEstado = adventiciosEstado;
    }
  
    if (cooperadoresEstado.length === 0) {
      delete estadoToSend.cooperadoresEstado;
    } else {
      estadoToSend.cooperadoresEstado = cooperadoresEstado;
    }

    if (dentCrossesEstado.length === 0) {
      delete estadoToSend.dentCrossesEstado;
    } else {
      estadoToSend.dentCrossesEstado = dentCrossesEstado;
    }
  
    if (odMedsEstado.length === 0) {
      delete estadoToSend.odMedsEstado;
    } else {
      estadoToSend.odMedsEstado = odMedsEstado;
    }
  
    if (rioPaxesEstado.length === 0) {
      delete estadoToSend.rioPaxesEstado;
    } else {
      estadoToSend.rioPaxesEstado = rioPaxesEstado;
    }
  
    if (dependentesEstado.length === 0) {
      delete estadoToSend.dependentesEstado;
    } else {
      estadoToSend.dependentesEstado = dependentesEstado;
    }
  
    try {
      const newEstado = await createEstado({ ...estado, adventicios, cooperadores, dependentes, dentCross, odMed, rioPax } as Estado);
      dispatch(addEstadoSuccess(newEstado));
      navigate('/estados');
    } catch (err: any) {
      ...
      const errorMessage = Array.isArray(errorResponse?.message)
        ...
        : errorResponse?.message || 'Erro ao adicionar estado.';
